% Comparación entre GA y PSO para la función Peaks
% preambulo
clear;clc; close all;
set(0,'DefaultLineLineWidth',2)
set(0, 'defaultfigurecolor', [1 1 1])

% Óptimo teórico de la función Peaks
% El mínimo global está aproximadamente en (0.228, -1.626) con valor ≈ -6.55
optimo_teorico_x = 0.228;
optimo_teorico_y = -1.626;
optimo_teorico_valor = peaks([optimo_teorico_x; optimo_teorico_y]);

fprintf('=== FUNCIÓN PEAKS - COMPARACIÓN GA vs PSO ===\n');
fprintf('Óptimo teórico: x = %.4f, y = %.4f, f = %.6f\n\n', ...
...
for i = 1:num_runs
    ...
    % Ejecutar GA
    tic;
    [x_ga, fval_ga, exitflag_ga, output_ga] = ga(@peaks, NV, optionsga);
    time_ga = toc;
    ...
    % Ejecutar PSO
    tic;
    [x_pso, fval_pso, exitflag_pso, output_pso] = particleswarm(@peaks, NV, lb, ub, options_pso);
    time_pso = toc;
...
% Calcular errores respecto al óptimo teórico
ga_errors = sqrt((ga_results(:,1) - optimo_teorico_x).^2 + (ga_results(:,2) - optimo_teorico_y).^2);
pso_errors = sqrt((pso_results(:,1) - optimo_teorico_x).^2 + (pso_results(:,2) - optimo_teorico_y).^2);
...
fprintf('  Error promedio desde óptimo: %.6f\n', mean(ga_errors));
...
sgtitle('Comparación GA vs PSO - Función Peaks');
