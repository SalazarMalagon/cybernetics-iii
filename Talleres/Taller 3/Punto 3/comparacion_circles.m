% Comparación entre GA y PSO para la función Circles
% preambulo
clear;clc; close all;
set(0,'DefaultLineLineWidth',2)
set(0, 'defaultfigurecolor', [1 1 1])

% Óptimo teórico de la función Circles
% f(x,y) = sin(x)^2 + sin(y)^2 + 0.1*(x^2 + y^2)
% El mínimo global está en (0,0) con valor f(0,0) = 0
optimo_teorico_x = 0;
optimo_teorico_y = 0;
optimo_teorico_valor = circles([optimo_teorico_x; optimo_teorico_y]);

fprintf('=== FUNCIÓN CIRCLES - COMPARACIÓN GA vs PSO ===\n');
...
optionsga = optimoptions('ga', ...
    'Display', 'off', ... % Se puede cambiar a 'off' para comparación limpia
    'PopulationSize', 25, ...
    'MaxGenerations', 200); ...
    %, ...
...
for i = 1:num_runs
    ...
    % Ejecutar GA
    tic;
    [x_ga, fval_ga, exitflag_ga, output_ga] = ga(@circles, NV, optionsga);
    time_ga = toc;
    ...
    % Ejecutar PSO
    tic;
    [x_pso, fval_pso, exitflag_pso, output_pso] = particleswarm(@circles, NV, lb, ub, options_pso);
    time_pso = toc;
...
sgtitle('Comparación GA vs PSO - Función Circles');
